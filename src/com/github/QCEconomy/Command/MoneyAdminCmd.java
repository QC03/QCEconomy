...
public class MoneyAdminCmd implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String s, String[] args) {
		...
		if (args[0].equals("확인"))
		{
			money = MoneyDatabase.getUserMoney(uuid);
		...
		if (args[0].equals("설정"))
		{
			MoneyDatabase.setUserMoney(uuid, money);
			sender.sendMessage(getMoneyPrefix() + name + " 님의 보유 돈을 " + NumberFormat.getInstance().format(money) + " 원 (으)로 설정하였습니다.");
			return true;
		}
		
		
		int userMoney = MoneyDatabase.getUserMoney(uuid);
		
		if (args[0].equals("지급"))
		{
			
			MoneyDatabase.setUserMoney(uuid, (userMoney + money));
